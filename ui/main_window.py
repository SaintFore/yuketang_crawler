...
class YuketangApp(ctk.CTk):
    ...
    def _create_widgets(self):
        ...
        self.status_label = ctk.CTkLabel(
            self.main_frame, text="准备就绪", font=ctk.CTkFont(size=14)
        )
        ...
        self.console.grid(
            row=13, column=0, padx=20, pady=10, sticky="ew"
        )  # 行号从10改为13

    def _create_step1_widgets(self):
        """创建步骤1相关的UI组件"""
        self.step1_label = ctk.CTkLabel(
            self.main_frame, text="步骤1: 输入雨课堂试卷ID", font=ctk.CTkFont(size=16)
        )
    ...
    def _create_step2_widgets(self):
        ...
        self.step2_label = ctk.CTkLabel(
            self.main_frame, text="步骤2: 启动代理并访问试卷", font=ctk.CTkFont(size=16)
        )
    ...
    def _create_step3_widgets(self):
        ...
        self.step3_label = ctk.CTkLabel(
            self.main_frame, text="步骤3: 提取试卷内容", font=ctk.CTkFont(size=16)
        )
        ...
        self.reorder_label = ctk.CTkLabel(
            self.reorder_frame, text="是否重新排序选项(ABCD顺序):"
        )
        self.reorder_label.grid(row=0, column=0, padx=10, pady=10, sticky="w")

        self.reorder_yes = ctk.CTkRadioButton(
            self.reorder_frame, text="是", variable=self.reorder_var, value="y"
        )
        self.reorder_yes.grid(row=0, column=1, padx=10, pady=10)

        self.reorder_no = ctk.CTkRadioButton(
            self.reorder_frame, text="否", variable=self.reorder_var, value="n"
        )
        self.reorder_no.grid(row=0, column=2, padx=10, pady=10)

        self.process_button = ctk.CTkButton(
            self.main_frame, text="提取试卷内容", command=self.process_json
        )
    ...
    def _create_step4_widgets(self):
        ...
        self.step4_label = ctk.CTkLabel(
            self.main_frame, text="步骤4: 提取答案到CSV", font=ctk.CTkFont(size=16)
        )
    ...
    def start_proxy(self):
        ...
        self.console.insert(
            "end", f"==== 启动代理会话 [{time.strftime('%H:%M:%S')}] ====\n"
        )
    ...
    def stop_proxy(self):
        ...
        if not self.console.get("1.0", "end").strip():
            self.console.insert(
                "end", f"==== 终止代理会话 [{time.strftime('%H:%M:%S')}] ====\n"
            )
    ...
    def save_answers_to_csv(self):
        ...
        self.console.delete("0.0", "end")
        self.console.insert(
            "end", f"==== 提取答案数据 [{time.strftime('%H:%M:%S')}] ====\n"
        )
