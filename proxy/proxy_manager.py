...
class ProxyManager:
    ...
    def proxy_open():
        """启用系统代理"""
        proxypre = ProxyManager.proxypre
        proxy = ProxySetting()
        proxy.registry_read()
        proxypre.append(proxy.enable)
        proxypre.append(proxy.server)
        proxypre.append(proxy.override)
        proxy.enable = True
    ...
    def proxy_restore():
        """恢复系统代理到之前的状态"""
        if ProxyManager.proxypre:
            proxy = ProxySetting()
            proxy.enable = ProxyManager.proxypre[0]
            proxy.server = ProxyManager.proxypre[1]
            proxy.override = ProxyManager.proxypre[2]
            proxy.registry_write()
    ...
    def kill_process_by_port(port):
        ...
        try:
            ...
            if result:
                ...
                for pid in pids:
                    sys.stdout.write(
                        f"[端口] 正在终止占用端口 {port} 的进程 (PID: {pid})\n"
                    )
                    sys.stdout.flush()
                    try:
        ...
        def run_proxy():
            try:
                ...
                if ProxyManager.is_port_in_use(port):
                    sys.stdout.write(
                        f"[警告] 端口 {port} 已被占用，尝试终止占用进程...\n"
                    )
                    sys.stdout.flush()

                    if status_callback:
                        status_callback(
                            f"端口 {port} 被占用，正在尝试终止...", "orange"
                        )
                    ...
                    if ProxyManager.is_port_in_use(port):
                        sys.stdout.write(
                            f"[错误] 端口 {port} 仍然被占用，无法启动代理\n"
                        )
                        sys.stdout.flush()
                        if status_callback:
                            status_callback(
                                f"端口 {port} 仍然被占用，请手动关闭占用进程", "red"
                            )
                        return
                ...
                script_path = os.path.abspath(
                    os.path.join(os.path.dirname(__file__), "yuketang_proxy.py")
                )
                sys.stdout.write(f"[配置] 脚本绝对路径: {script_path}\n")
                sys.stdout.write(
                    f"[配置] 脚本是否存在: {os.path.exists(script_path)}\n"
                )
                sys.stdout.flush()
                ...
                if not os.path.exists(log_dir):
                    os.makedirs(log_dir)
                with open(
                    os.path.join(log_dir, "error.log"), "a", encoding="utf-8"
                ) as f:
    ...
    def stop_proxy(status_callback=None):
        ...
        """
        try:
            ProxyManager.proxy_restore() # 恢复系统代理到之前的状态
                    ...
                    if platform.system() == "Windows":
                        subprocess.run(
                            [
                                ...
                                "/T",
                                "/PID",
                                str(ProxyManager.current_process.pid),
                            ],
                            shell=True,
